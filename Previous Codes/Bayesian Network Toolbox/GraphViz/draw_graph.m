function [x, y, h] = draw_graph(adj, labels, adj_type, node_t, x, y, varargin)
...
%       ADJ_TYPE : types of edges in the adjacency matrix
...
for i=1:N,
    ...
    for k=j,
        ...
        if adj(k,i)==0, % if directed edge
            h = arrow([x(i)+dx1 y(i)+dy1],[x(k)-dx2 y(k)-dy2],'BaseAngle',30, 'Color','black');
%             h_dedge = textbox( (x(i)+dx1 + x(k)-dx2)/2, (y(i)+dy1 + y(k)-dy2)/2, num2str(adj_type(k,i)) );
%             set(h_dedge(:,2),'EdgeColor','white')
        else
            h = line([x(i)+dx1 x(k)-dx2],[y(i)+dy1 y(k)-dy2],'Color','black');
%             h_dedge = textbox( (x(i)+dx1 + x(k)-dx2)/2, (y(i)+dy1 + y(k)-dy2)/2, num2str(adj_type(k,i)) );
%             set(h_dedge(:,2),'EdgeColor','white')
            %         y1 = y(i)+dy1;
            %         y2 = y(k)-dy2;
            %         if y1 ~= y2 %different levels
            %             h = line([x(i)+dx1 x(k)-dx2],[y(i)+dy1 y(k)-dy2],'Color','white');
            %         else
            %             x1 = x(i)+dx1;
            %             x2 = x(k)-dx2;
            %             range_x = x2-x1;
            ...
            %             x4 = x1 + range_x / 2;
            %             x5 = x1 + 3*range_x / 4;
            %             temp_x = [x1,x3,x4,x5,x2];
            %             temp_y = [y1, y1- 0.05, y1-0.1, y1-0.05, y2];
            %             xValues = linspace(x(i)+dx1,x(k)-dx2);
            %             yValues = interp1(temp_x,temp_y,xValues,'pchip');
            %             h = line(xValues,yValues,'Color','white'); %to have curved lines
            %         end
