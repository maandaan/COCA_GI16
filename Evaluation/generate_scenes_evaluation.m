function generate_scenes_evaluation( objects_num, input_scene_filename, ...
    results_filename, max_arrangement_opt_iter )
%GENERATE_SCENES_EVALUATION a second version of the function
%generate_scenes.m with some minor changes for arrangement evaluation
%purposes.
...
%     
%     %check how symmetry affects the 3D model selection process
%     orig_scene = scene;
...
%     
%     %putting back the symmetry relations for the arrangement
%     for oid = 1:length(scene)
%         obj = orig_scene(oid);
%         if ~isempty(obj.symm_group_id)
%             scene(oid).symm_group_id = obj.symm_group_id;
%             if ~isempty(obj.symm_ref_id)
%                 scene(oid).symm_ref_id = obj.symm_ref_id;
%             end
%         end
%     end
%     
...
%     save([scenes_dir, results_filename, '_init_symmetrymodelselection_', num2str(sample_id)], 'scene');
%     fprintf('Finished initializing the placement and scaling the sizes for sample %d!\n', sample_id);
    ...
    save([scenes_dir, results_filename, '_final_symmorientarrangement_', num2str(sample_id)], 'final_scene');
    ...
    out_file = [scenes_dir, results_filename, '_symmorientarrangement_', num2str(sample_id), '.txt'];
