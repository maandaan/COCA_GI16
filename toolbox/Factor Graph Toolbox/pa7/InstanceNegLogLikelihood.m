...
%     load('Part2Sample.mat', 'sampleModelFeatureCounts');
%     ddd=1;         
...
function modelFeatureCounts = getModelFeatureCounts(theta, featureSet, marginalizedFactors, numFtrs, numCharacters)
    ...
    for i = 1:numFtrs
        ...
        elseif(length(featureSet.features(i).var) == 2)
...
%             if(all(ismember([k, k+1], P.cliqueList(i).var)))
                ...
                if any(marginalizedFactors(numCharacters + k).val ~= 0)
                    % Normalize
                    marginalizedFactors(numCharacters + k).val = marginalizedFactors(numCharacters + k).val/sum(marginalizedFactors(numCharacters + k).val);
                end
                break;
            end
        end
...
function [featureCounts, weightedFeaturCounts] = getWeightedFeaturCounts2(y, theta, featureSet, numFtrs)
    ...
    for n = 1:numFtrs
        ...
        weightedFeaturCounts(paramIdx) = weightedFeaturCounts(paramIdx) + theta(featureSet.features(n).paramIdx) * currentFtrCount;
    end
end
