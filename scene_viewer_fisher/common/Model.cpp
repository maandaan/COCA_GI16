...
Model::Model(const std::string &directory, const std::string &baseFilename, Assets &textureStore)
{
    ...
    // Compile meshes
    //
    _displayList = glGenLists(1); // we want 1 identifier for the list and the identifier will be returned in displaylist
    
    glNewList(_displayList, GL_COMPILE); // create the list, GL_COMPILE means saving the commands for later and not executing them immediately
    {
...
void Model::Render()
{
    glCallList(_displayList); // execute the display list
}
